...
function setupTooptips() {
  ...
  tooltipWidget("modList", qsTr("This is the mod install list section. This window shows all the mods that are installed. The column headers can be used for sorting. Only checked mods are active in the current profile."))
  tooltipWidget("profileBox", qsTr("Displays active profile, select to manage or switch profiles. Each profile is a separate set of enabled mods and ini settings and can be configured to share ini settings and game saves."))
  ...
  tooltipWidget("restoreModsButton", qsTr("Restore a mod install list backup."))
  ...
  tooltipWidget("groupCombo", qsTr("The dropdown allows various ways of grouping the mods shown in the mod install list."))
  tooltipWidget("displayCategoriesBtn", qsTr("Show/hide the category pane."))
  tooltipWidget("modFilterEdit", qsTr("Quickly filter the mod install list as you type."))
  ...
  tooltipWidget("startButton", qsTr("When this button is clicked, Mod Organizer 2 creates a virtual directory structure then runs the program selected to the left."))
  ...
  tooltipWidget("logList", qsTr("Log messages produced by MO2. Please note that messages with a light bulb usually don't require your attention."))
  ...
  tooltipAction("action_Refresh", qsTr("Refresh MO2 to detect changes."))
  ...
  tooltipAction("actionSettings", qsTr("Configure Mod Organizer 2."))
  ...
  tooltipAction("actionUpdate", qsTr("Activates if there is an update for MO2. Please note that if, for any reason, MO2 can't communicate with NMM, this will not work either."))
  ...
  switch (tutorialControl.getTabName("tabWidget")) {
    case "espTab":
      tooltipWidget("espList", qsTr("Plugin load order (esp/esm/esl files) of the mods in the current profile. They need to be checked to be loaded."))
      tooltipWidget("sortButton", qsTr("Automatically sort plugins using the bundled LOOT application."))
      tooltipWidget("restoreButton", qsTr("Restore a backup of your plugin load order."))
      tooltipWidget("saveButton", qsTr("Save a backup of your plugin load order."))
      tooltipWidget("activePluginsCounter", qsTr("Counter of your total active plugins. Hover to see a breakdown of plugin types."))
      tooltipWidget("espFilterEdit", qsTr("Quickly filter plugin load order as you type."))
      break
    case "bsaTab":
